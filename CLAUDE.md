...
| `FIXTURES`, `RESULTS` | Per-round schedule and results. `FIXTURES` teams, `KICKOFF`, `ROUND`, `SEASON` and `DERBY` are NOT NULL (`SET_NOT_NULL_FIXTURES`); `LOCATION` is not. `PICKABLE` (`ADD_PICKABLE_TO_FIXTURES`, default true) marks a fixture the engine has ruled out of the deadline calculation because it kicks off at or before the end of the previous round; the API will not let anyone pick into one. `RESULTS` (`HOME_GOALS`, `AWAY_GOALS`, `WINNER`, `GAME_STATUS`) holds a row for every fixture of a closed round, with NULL goals and `WINNER` when the match was not finished |
| `ROUNDS` | Round metadata: `CUT_OFF`, `DP_ROUND`, `DMM_ROUND` |
| `CURRENT_ROUND` | Singleton: `ROUND_ID`, `SEASON`, `OFF_SEASON`, `NEXT_SEASON_DATE` |
| `CHOICES` | One pick per player per round; `METHOD` marks auto-assigned picks; `(ROUND, SEASON)` is a foreign key to `ROUNDS`. `MADE_AT` (`ADD_MADE_AT_TO_CHOICES`) is when the pick currently held was made, in UTC: filled by its `UTC_TIMESTAMP()` default on insert (fpg-api's first pick, fpg-engine's auto-assign) and moved by fpg-api when a pick changes team. NULL for every pick made before the migration reached that schema, and never estimated |
...
Most game tables carry a `SEASON` column. Foreign keys: `RESULTS` to `FIXTURES`; `CHOICES`, `SCORES`, `MINI_LEAGUE_SCORES` and `MINI_LEAGUE_STANDINGS` to `ROUNDS` on `(ROUND, SEASON)`; `MINI_LEAGUE_MEMBERS`, `MINI_LEAGUE_SCORES` and `MINI_LEAGUE_STANDINGS` to `MINI_LEAGUES`; and `PLAYER_ID` keys to `USERS` from `CHOICES`, `SCORES`, `STANDINGS`, `MINI_LEAGUES` (`CREATED_BY`), the three mini-league child tables, `REFRESH_TOKENS`, `TOKENS`, `MCP_TOKENS` and the three OAuth token tables. Delete rule: sessions and memberships (`REFRESH_TOKENS`, `TOKENS`, MCP and OAuth tokens, `MINI_LEAGUE_MEMBERS`) cascade from `USERS`, and league children cascade from `MINI_LEAGUES`; game history and league ownership restrict, so an account with picks cannot be deleted. Nothing deletes accounts today; deactivation sets `IS_DISABLED`. There is no key from `FIXTURES` to `ROUNDS`, because a season's fixtures are loaded up front while the engine creates each round's row as it opens. Until the reconcile in `2026-09-fixture-reconciliation` ships, that load is the only thing that ever writes `FIXTURES`, and it never revisits a row (FPG-APP/fpg-docs#78).

Time columns are `DATETIME` with whole seconds; `TIMESTAMP` is not used. Audit columns (`created_at`, `expires_at`, `LOGS.TIME_ADDED`, `CALL_LOGS.call_time` and the like) are filled by `CURRENT_TIMESTAMP` or `NOW()` in the server's time zone, which is `SYSTEM` (Europe/London). `FIXTURES.KICKOFF` and `ROUNDS.CUT_OFF` are UTC as api-football supplies them, and `CHOICES.MADE_AT` is UTC so it compares directly with them. Both families leave the database without a zone; fpg-api marks the two football columns as UTC on the way out (FPG-APP/fpg-docs#64), and the server cannot convert between them in SQL because `CONVERT_TZ` returns NULL here.
...
- `populate_dev.sql` copies `FPG.*` into the current schema, so `DEV_FPG` is ahead of `FPG` for as long as a migration is being rehearsed, which is exactly when the script gets run. A `SELECT *` copy of a table whose shape has just changed fails with "Column count doesn't match value count" *after* the truncate, leaving that table empty, and the migration is then marked unapplied, so `yoyo apply` fixes it, not `reapply`, which is a silent no-op on an unapplied migration. The fix is to name that table's columns for as long as the window is open, and put it back to `SELECT *` once production has caught up. No copy is named today. When the new column has a default, the named copy must also write that column explicitly (NULL, or whatever production holds): a column left out of an `INSERT` takes its default, which for `CHOICES.MADE_AT` would have stamped every copied pick with the time of the copy. `CHOICES` was named for `MADE_AT` and `FIXTURES` and `LOGS` for `PICKABLE` and `RUN_ID`, and all three went back on 2026-09-16; `CHOICES` had been named once before, for `FIXTURE_ID`, and went back on 2026-09-11.
- `populate_dev.sql` is not in the repo: it is gitignored beside `yoyo.ini` and exists only on the laptop and as an applied row in `DEV_FPG._yoyo_migration`. It truncates seventeen tables (`USERS`, `TOKENS` and the four mini-league tables among them) and copies `FPG.*` into the current schema inside a `SET FOREIGN_KEY_CHECKS = 0` / `= 1` pair, because a parent of a foreign key cannot be truncated otherwise, then deletes rows in the session tables it does not copy (`REFRESH_TOKENS`, `MCP_TOKENS`, the OAuth tables) whose account is not in the copied `USERS`, so the keys to `USERS` hold; every copy is `SELECT *` again since 2026-09-16, so a migration rehearsed in `DEV_FPG` first needs its table's copy named for the length of the window, for the reason above. It is otherwise column-order dependent and would wipe production if ever applied to `FPG`; only reapply it against `DEV_FPG`.
