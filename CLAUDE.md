...
- Start every new file with `-- depends: <current chain tail>` so Yoyo orders it after everything else. The chain tail is `ADD_PICKABLE_TO_FIXTURES`, the last of a twelve-file chain that starts at `UPDATE_USERS_CREATED_AT_FROM_PLAYERS` (which depends on `SET_NOT_NULL_FIXTURES`). About twenty older files have no header and rely on scan order; do not add to that set.
